# -*- coding: gbk -*-
# @Time : 2023/3/31 21:27
# @Author : wufei
# @File : aspect_finetune_restaurants.py
...
from model.module.bert_post_In_new import BertMLMHead, BertPostLayerNormalizationModel, ABSAOutput
...
def collate_fn(batch):
    bert_tokens, aspect_masks, labels, raw_texts, raw_aspect_terms = zip(*batch)
...
def list2txt(file_path, list):
    with open(file_path, 'w') as f:
        f.write(str(list))


def evaluate(config, model, data_loader, epoch, criterion=None):
    ...
    with torch.no_grad():
        for batch in data_loader:
            bert_tokens, bert_masks, aspect_masks, labels, raw_texts, raw_aspect_terms = batch
    ...
    for epoch in range(config['epoch']):
        ...
        for idx, batch in enumerate(train_loader):
            ...
            if idx % valid_frequency == 0 and idx != 0:
                ...
                val_acc, val_f1, val_loss = evaluate(config, model, test_loader, epoch,
...
if __name__ == '__main__':
    config = {'mode': 'aspect_finetune', 'model': 'BERT', 'device': 0, 'data_path': 'data/laptops',
              ...
              'train_file': '/Data/restaurants/Restaurants_Train_v2_Implicit_Labeled_preprocess_finetune.pkl',
              'test_file': '/Data/restaurants/rest_test_enriched_preprocess_finetune.pkl',
              'num_workers_per_loader': 0,
              'seed': 42, 'batch_size': 32, 'dropout': 0.1, 'epoch': 6, 'warm_up': 70, 'label_smooth': 0.0205,
              'optimizer': 'adamw', 'learning_rate': 5e-05, 'weight_decay': 0.015, 'grad_norm': 5.0,
              'checkpoint': '/Data/BERT_laptops.pt'}
